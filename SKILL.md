...
- **Real photography.** Ground real subjects in real photos. If hosting images for a sandboxed-iframe preview, embed a public absolute URL — thread-scoped `/api/files/...` URLs can't authenticate inside the iframe (broken-image trap).
